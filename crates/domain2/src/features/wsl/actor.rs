...
use app_contracts2::features::wsl::{
...
impl<P: WslPort> std::fmt::Debug for WslActor<P> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("WslActor")
            .field("distros", &self.distros.len())
            .field("configured", &self.configured)
            .field("machine", &self.machine.is_some())
            .finish()
    }
}
    ...
    match msg {
        RemoteScanResult::Scan(scan) => {
            ...
            this.machine = Some(LinuxMachineSummary {
                cpu_percent: percent,
                memory_used_bytes: scan.machine.used_kb * 1024,
                memory_total_bytes: scan.machine.total_kb * 1024,
