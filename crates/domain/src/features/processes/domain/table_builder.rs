use crate::processes_impl::services::metadata::{ProcessMetadata, ProcessMetadataService};
...
impl<'a> TableDataBuilder<ProcessNodeDto, ProcessEntryVm> for ProcessTreeBuilder<'a> {
    fn build_tree(
        ...
        while i < buffer.len() {
            ...
            let mut parent_vm = to_vm(self.metadata, &items[leader_idx], 0, self.show_icons);
                ...
                for &(_, idx) in group_procs {
                    if idx != leader_idx {
                        children.push(TableNode {
                            vm: to_vm(self.metadata, &items[idx], 1, self.show_icons),
                            group_id: None,
                            has_children: false,
                            is_expanded: false,
                            level: 1,
...
fn to_vm(
    ...
    ProcessEntryVm {
        ...
        icon: if show_icons {
            proc.exe_path
                .as_ref()
                .map(|p| metadata.icon_by_path(p))
                .unwrap_or_default()
        } else {
            metadata.icon_by_path("none")
        },
        depth,
        has_children: false,
        is_expanded: false,
