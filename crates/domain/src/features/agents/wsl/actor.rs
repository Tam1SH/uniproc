...
impl WslAgentActor {
    ...
    fn apply(&mut self, event: ConnectionEvent) -> Option<Transition> {
        match self.connection.apply(event) {
            Ok(t) => Some(t),
            Err(err) => {
                warn!("WSL FSM invalid: {:?} on {:?}", err.event, err.state);
                None
            }
        }
    }
    ...
    fn publish_state(&self, latency_ms: Option<i32>) {
        ...
        EVENT_BUS.with(|bus| bus.publish(WslAgentRuntimeEvent { state, latency_ms }));
    }
}
...
impl<TWindow: ComponentHandle + 'static> Handler<Init, TWindow> for WslAgentActor {
    #[instrument(skip(self, ctx))]
    fn handle(&mut self, _: Init, ctx: &Context<Self, TWindow>) {
        info!("WslAgentActor init");
        self.publish_state(None);
        ctx.addr().send(StartConnect);
    }
}

impl<TWindow: ComponentHandle + 'static> Handler<ScanTick, TWindow> for WslAgentActor {
...
impl<TWindow: ComponentHandle + 'static> Handler<ReportResult, TWindow> for WslAgentActor {
...
impl<TWindow: ComponentHandle + 'static> Handler<StartConnect, TWindow> for WslAgentActor {
...
impl<TWindow: ComponentHandle + 'static> Handler<ConnectResult, TWindow> for WslAgentActor {
...
impl<TWindow: ComponentHandle + 'static> Handler<TryConnectWithDelay, TWindow> for WslAgentActor {
...
impl<TWindow: ComponentHandle + 'static> Handler<RetryTimerElapsed, TWindow> for WslAgentActor {
...
impl<TWindow: ComponentHandle + 'static> Handler<Ping, TWindow> for WslAgentActor {
...
impl<TWindow: ComponentHandle + 'static> Handler<PingResult, TWindow> for WslAgentActor {
...
impl<TWindow: ComponentHandle + 'static> Handler<ConnectionLost, TWindow> for WslAgentActor {
